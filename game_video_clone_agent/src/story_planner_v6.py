...
# ── 文案质量闸门常量 ──
# 目标 80 镜，低于 CUT_MIN 视为不达标，触发重试。
# 三次重试仍不达标，则采用候选中 CUT 最多（最接近 80 镜）的一份继续流水线，
# 避免在 LLM 长期波动时整条流水线无限阻塞。
CUT_TARGET = 80
CUT_MIN = 70
...
【stage_map 硬性约束】（解决"幼年/老年定妆照不被采用"的根因）：
1. stage_map 的 stage 字段只能在 ["child", "youth", "middle", "elderly"] 中取值（与 physical_char_anchors 完全对齐，禁止使用 "old" 等同义词）。
2. stage_map 必须覆盖全部 80 个分镜：第一项的 start_shot 必须为 1，最后一项的 end_shot 必须为 80。
3. 区间必须连续、无缺口、不重叠（前一项 end_shot + 1 = 后一项 start_shot）。
4. 仅列出剧情中真正出现的人生阶段（如剧情中没有幼年戏，就别强加 child）。
...
请输出 JSON 格式（内部键名固定，不可变更）：
{
  ...
  "stage_map": [{"stage": "child", "start_shot": 1, "end_shot": 25}, {"stage": "middle", "start_shot": 26, "end_shot": 60}, {"stage": "elderly", "start_shot": 61, "end_shot": 80}]
}
"""
...
def _count_cut_tags(narration: str) -> int:
    """统计旁白中 [CUT_XXXX] 标签数量。"""
    if not narration:
        return 0
    return len(re.findall(r"\[CUT_\d+\]", narration))


def expand_narration_once(synopsis_data: dict, topic: str):
    """阶段 2：单次调用 LLM 扩写沉浸式旁白 (锁死 1700 字和 80 镜)"""
    client = get_client()
    try:
...
def expand_narration(synopsis_data: dict, topic: str):
    """
    阶段 2：扩写沉浸式旁白，并叠加质量闸门。

    新增逻辑（解决"视频时长偏短"的根因）：
      1) 每次调用后立即统计 [CUT_XXXX] 数量；
      2) >= CUT_MIN 直接采纳；
      3) < CUT_MIN 记录为候选并重试，最多 NARRATION_MAX_RETRIES 次；
      4) 仍不达标则返回候选中 CUT 最多（最接近目标）的一份，
         避免下游因短文案静默缩水或卡死。
    """
    print(f"\n[Step 2] 正在扩写 {CUT_TARGET} 镜沉浸式文案 (目标 1700-1900 字, CUT 下限 {CUT_MIN})...")

    best_result = None
    best_cut_count = -1

    for attempt in range(1, NARRATION_MAX_RETRIES + 1):
        print(f"  └─ [Narration Attempt {attempt}/{NARRATION_MAX_RETRIES}] 调用 LLM...")
        result = expand_narration_once(synopsis_data, topic)
        if not result:
            print(f"     ⚠️  第 {attempt} 次调用未返回有效 JSON，进入下一次重试。")
            continue

        narration = result.get("full_narration", "")
        cut_count = _count_cut_tags(narration)
        char_count = len(re.sub(r"\[CUT_\d+\]", "", narration))
        print(f"     · CUT 数={cut_count} (目标 {CUT_TARGET}, 下限 {CUT_MIN})  字数≈{char_count}")

        if cut_count > best_cut_count:
            best_result = result
            best_cut_count = cut_count

        if cut_count >= CUT_MIN:
            print(f"     ✅ 通过文案质量闸门（CUT={cut_count} ≥ {CUT_MIN}）")
            return result

        print(f"     ❌ CUT 数量不达标（{cut_count} < {CUT_MIN}），准备重试...")

    if best_result is None:
        print("❌ 文案生成连续失败，无可用候选。")
        return None

    print(
        f"⚠️  连续 {NARRATION_MAX_RETRIES} 次未达标，采用候选最佳结果继续流水线 "
        f"(CUT={best_cut_count})，建议人工复核或重跑。"
    )
    return best_result
