...
target_path = r"c:\Users\86198\Desktop\Automated-storytelling-main\game_video_clone_agent\api_server.py"
...
def extract_entities_manually(text: str, mode_path: str = "RED") -> List[str]:
    \"\"\"
    智能提炼定妆照核心实体。
    - 剧情故事 (RED/BLUE): 提炼 3 个词 [最核心人物角色, 核心大场景, 最关键线索道具]
    - 硬核科普 (YELLOW): 仅提炼 1 个词 [最适合的科学演示大板绘背景]
    \"\"\"
    client = planner.get_client()
    
    if mode_path == "YELLOW":
        print("[API 中控] 科普频道：正在为定妆照智能提炼唯一的【科学板绘背景基底】实体词...")
        sys_prompt = "你是一个视频美术指导。请读下方科普文本，提炼出最适合作为全片科学演示大背景底板的【1个视觉基底风格词】（如：\\\"深色物理网格草稿纸\\\" 或 \\\"科技蓝图电路图背景\\\" 或 \\\"石板手绘黑板背景\\\"）。只返回一个中文 JSON 数组，只包含这1个词（如：[\\\"深色物理网格草稿纸\\\"]），不要任何解释和 markdown 围栏。"
    else:
        print("[API 中控] 剧情频道：正在为定妆照智能提炼核心三要素...")
...
def health_check():
    \"\"\"保活心跳，用于前端红绿灯指示\"\"\"
    return {"status": "success", "msg": "Palette Cinema Local API is live and kicking!"}
...
async def single_frame_render(req: SingleFrameRenderRequest):
    ...
    try:
        ...
        if req.target_id.startswith("cast_"):
            ...
            # 根据当前的卡片 id 返回正确的 svgType
            stype = "character"
            if req.target_id == "cast_01":
                stype = "scene" if len(GLOBAL_STATE.get("entities", [])) == 1 else "character"
            elif req.target_id == "cast_02":
                stype = "scene"
            elif req.target_id == "cast_03":
                stype = "prop"
...
async def synthesize_video():
    ...
    print(f"\\n[API 中控] 正在为批次 {run_id} 启动全盘视频合成流程...")
...
with open(target_path, "w", encoding="utf-8") as f:
    f.write(code)
print("api_server.py completely written successfully!")
